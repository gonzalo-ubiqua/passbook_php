  ...
  private function generatePassBooks($input_array) {
    ...
    for ($row=0; $row<count($input_array); $row++) {
      ...
      try {
        ...
        $output  = '';
        $command = "java -cp passbook-1.0-jar-with-dependencies.jar me.compare.passbook.App $sec '$name' 2> /dev/null";
        $this->ret_data->log ( "Voy a llamar al jar jar: " . $command );
        try {
          exec($command, $output);
        } catch (Exception $e) {
          $this->ret_data->log ( "Error llamanda a jar: " . $e->getMessage() );
        }
